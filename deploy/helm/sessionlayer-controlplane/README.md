...
| `networkPolicy.extraGrpcIngressCidrs` | `[]` | Agents reaching `mtls.port` from an address no pod selector can express: fleet nodes outside the cluster, **and the in-cluster Agent DaemonSet**, which requires `hostNetwork` and so arrives from the node address. Without your node ranges here, those Agents fail their join with `transport error` and nothing appears in this Control Plane's log. |
...
Also proven for this chart specifically: rendering refuses without
`secrets.existingSecret`, naming every key the Secret must carry; the Control
Plane refuses to boot on its built-in dev KEK; and with a real KEK, Flyway
applied the schema and all four certificate authorities were generated
and wrapped under the operator's KEK (`kek_reference` =
`config:sessionlayer.ca.local.kek-base64`, not `dev-default:insecure`). Two
replicas ran behind one Service, which is what surfaced the machine-token
signing defect that install fixed.
