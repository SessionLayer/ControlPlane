...
```
cd framegen && cargo run          # rewrites both files; commit them together
```
...
`framegen` is a manual dev tool that **never runs in CI**, and `vendor-contracts.sh
--check` only verifies a vendored copy against the canonical source — so nothing
...
That hole is closed at the source, in the **ControlPlane gate** (the only place
...
mismatch) **fails the ControlPlane gate**, forcing a regen. Both
