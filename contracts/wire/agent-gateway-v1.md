...
```
  ...
  [session for node N authorized; connector_kind=OUTBOUND_AGENT]
     ...
     |                                                        |
     |  [ByteStream handed to the inner leg - host verify,   |
...
- `signature` = ECDSA **P-256 / SHA-256** over
  ...
  signature can never be lifted from another context (mirrors the
  decision-context signer's domain separation).
...
- **Credential rotation.** The Agent's identity is renewed by its renew-ahead
  loop. The control channel observes the rotation and reconnects with the
