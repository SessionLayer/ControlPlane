...
# it can sign OpenSSH user certificates (the SshCertSigner Vault backend).
# Idempotent. Prints the CA public key on stdout (feed it to a node's
# TrustedUserCAKeys). Uses POST /ssh/sign, never /ssh/issue: the requester submits
# a public key, so no private key is ever generated by or returned from Vault.
#
