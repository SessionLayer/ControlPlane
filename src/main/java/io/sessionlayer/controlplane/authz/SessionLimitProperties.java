...
/**
 * Session-limit configuration ({@code sessionlayer.session-limits.*}).
 *
 * <ul>
 ...
 *
 * <p>
 * Reap-safety invariant (held BY CONSTRUCTION): transient under-count recovers
 * via next extend (GREATEST guard); permanent loss requires CP partition &gt;
 * lease-extension/2 + reaper.grace. Grace floored to window; Gateway self-heal
 * wins partitions &lt; window+grace (~&gt;22.5m at defaults). Window floored
 * 60s (below ~10s collapses under Gateway 5s cadence).
 */
