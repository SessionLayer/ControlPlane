-- V6 — operator_settings (config). SessionLayer Control Plane.
--
-- `settings:write` (FR-PADM-1) guards this resource. This is the singleton
-- cluster-configuration entity that cold start (Design §2A / §5.5, FR-BOOT-1)
-- reads to provision the three CAs, and that the application reads for
-- retention/WORM/OTP/session-limit defaults and the FR-BOOT-2 first-admin
-- bootstrap self-disable flag.
--
...
-- documented in docs/reference/data-model.md in the SessionLayer/Documentation repo.
-- Cold start writes this row at first boot.
--
...
CREATE TABLE config.operator_settings (
    ...
    -- config.session_limit_policy (V10); the enforcement semaphore lives in the
    -- application layer.
    ...
    -- FR-BOOT-2 first-admin bootstrap (data only; the bootstrap FLOW is in the application).
