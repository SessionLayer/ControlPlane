...
-- object-store version (Design §15 crown jewels).
...
COMMENT ON COLUMN runtime.recording_ref.object_version_id IS '§15: object-store version id of the finalized ciphertext object; replay/export pin it so a later shadow PUT to the same key is never served. Write-once once set (V24 trigger).';
