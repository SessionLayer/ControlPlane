...
-- Schema only; the owning behaviour (auth, session limits, node lifecycle) lives
-- in the application layer. Round-trip + constraint + schema-presence gates in
-- ModelGapSchemaIT.

-- ---------------------------------------------------------------------------
-- service_account_credential (FR-AUTH-12) — issued machine-consumer creds.
...
-- ---------------------------------------------------------------------------
-- device_flow (FR-AUTH-3/6, Design §5.2/§15) — OIDC device-flow state.
-- Stores HASHes of the device_code/user_code (never raw). connection_binding is the
-- 1:1 device_code<->connection anti-phishing binding (§15). Behaviour lives in
...
-- ---------------------------------------------------------------------------
-- node_host_key (FR-CONN-5, Design §9.3) — enrollment-anchored node host
...
-- ---------------------------------------------------------------------------
-- session_lease (FR-SESS-3) — cluster-wide concurrency primitive. One live
-- lease per active session; per-identity concurrency = count of unreleased leases.
-- The enforcement semaphore/lease-acquire logic lives in the application layer;
-- this is the durable primitive.
-- ---------------------------------------------------------------------------
