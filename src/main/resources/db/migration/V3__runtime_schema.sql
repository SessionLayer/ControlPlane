...
-- 1.1.1 has no cidr/inet codec that preserves a mask (see docs/reference/data-model.md
-- in the SessionLayer/Documentation repo), so
...
--     single addresses; network containment is computed at query time.
...
CREATE TABLE runtime.node (
    ...
    -- identity (FR-CONN-5) lives on node_policy / node_host_key, not here.
...
CREATE TABLE runtime.jit_request (
    ...
    -- self-approval (approver != requester, FR-ACC-4) invariant are enforced by the
    -- application.
...
-- Self-approval (approver != requester, FR-ACC-4) is a hard invariant enforced by
...
CREATE TABLE runtime.recording_ref (
    ...
    hash_chain_head    text,                                     -- recording hash-chain head (application-populated)
...
-- columns (prev_hash/record_hash) are populated by the application.
-- ---------------------------------------------------------------------------
CREATE TABLE runtime.audit_event (
    ...
    prev_hash      text,                                        -- hash chain (application-populated)
    record_hash    text,                                        -- hash chain (application-populated)
